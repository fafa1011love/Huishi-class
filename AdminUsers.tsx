...
function statusClass(statusCode: number | null) {
  if (statusCode === null) return 'bg-white/8 text-white/60';
  if (statusCode >= 500) return 'bg-red-500/15 text-red-100';
  if (statusCode >= 400) return 'bg-amber-400/15 text-amber-100';
  ...
  return (
    <div className="min-h-screen bg-[#071018] text-white">
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-cyan-300/25 bg-cyan-300/10 px-4 text-sm font-semibold text-cyan-50 transition hover:bg-cyan-300/20 hover:text-white"
            >
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6 flex w-full rounded-lg border border-white/10 bg-black/20 p-1 sm:inline-flex sm:w-auto">
          <button
            type="button"
            onClick={() => setActiveTab('users')}
            className={`inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-md px-4 text-sm font-bold transition sm:flex-none ${activeTab === 'users' ? 'bg-cyan-300 text-[#06212a]' : 'text-white/55 hover:bg-white/[0.06] hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-md px-4 text-sm font-bold transition sm:flex-none ${activeTab === 'logs' ? 'bg-cyan-300 text-[#06212a]' : 'text-white/55 hover:bg-white/[0.06] hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-md px-4 text-sm font-bold transition sm:flex-none ${activeTab === 'resources' ? 'bg-cyan-300 text-[#06212a]' : 'text-white/55 hover:bg-white/[0.06] hover:text-white'}`}
          >
            ...
            <section className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.03] shadow-2xl shadow-black/25">
              <div className="overflow-x-auto">
                <div className="min-w-[1365px]">
                  <div className={`grid ${USER_TABLE_GRID} gap-3 border-b border-white/10 bg-[#18212b] px-5 py-3 text-xs font-bold uppercase tracking-wide text-white/45`}>
                    <span>用户名</span>
                    <span>学校</span>
                    <span>角色</span>
                    <span>状态</span>
                    ...
                    <span>最近访问时间</span>
                    <span>最近访问 IP</span>
                    <span className="sticky right-0 z-20 -my-3 flex items-center justify-end border-l border-white/8 bg-[#18212b] py-3 pl-4 text-right shadow-[-12px_0_18px_rgba(3,10,18,0.35)]">操作</span>
                  </div>
                    ...
                    <div className="divide-y divide-white/8">
                      {users.map((user) => {
                        ...
                        return (
                          ...
                          >
                            <div className="min-w-0 truncate font-semibold text-white" title={user.username}>{user.username}</div>
                            <div className="min-w-0 truncate text-white/60" title={user.school || undefined}>{user.school || '-'}</div>
                            <div>
                            ...
                            <div className="whitespace-nowrap tabular-nums text-white/50">{formatDate(user.createdAt)}</div>
                            <div className="whitespace-nowrap tabular-nums text-white/50">{formatDate(user.lastAccessAt)}</div>
                            <div className="whitespace-nowrap font-mono text-xs text-white/50">{user.lastAccessIp || '-'}</div>
                            <div className="sticky right-0 z-10 -my-4 flex min-h-20 flex-nowrap items-center justify-end gap-2 border-l border-white/8 bg-[#0d1720] py-4 pl-4 shadow-[-12px_0_18px_rgba(3,10,18,0.35)]">
                              <button
                                type="button"
                                onClick={() => openUserLogs(user.id)}
                                className="inline-flex h-9 w-24 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-white/75 transition hover:bg-white/10 hover:text-white"
                              >
                              ...
                              <button
                                ...
                                className="inline-flex h-9 w-28 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-cyan-300/20 bg-cyan-300/10 px-3 text-xs font-bold text-cyan-100 transition hover:bg-cyan-300/18 disabled:cursor-not-allowed disabled:opacity-45"
                              >
          ...
          <section className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.03] shadow-2xl shadow-black/25">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-white/[0.04] px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs text-white/45">主信息会包含经批准的对话内容和限长建模提示词；不包含密码、凭证、图片数据、Cookie、Token 或完整请求正文。</p>
              </div>
              <label className="flex items-center gap-2 text-sm text-white/65">
                ...
                <select
                  ...
                  className="h-10 min-w-[190px] rounded-lg border border-white/10 bg-[#0b1a28] px-3 text-sm text-white outline-none focus:border-cyan-300/60"
                  aria-label="日志查看范围"
                >
            ...
            {logsMessage && (
              <div className="m-5 rounded-lg border border-red-300/25 bg-red-500/10 px-4 py-3 text-sm text-red-100">
                {logsMessage}
              </div>
            )}
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[1280px]">
                <div className="grid grid-cols-[1.15fr_1fr_3.3fr_1.8fr_0.7fr_1.1fr] gap-4 border-b border-white/10 px-5 py-3 text-xs font-bold uppercase tracking-wide text-white/45">
                  <span>时间</span>
                  <span>用户</span>
                  <span>用户行为（主要信息）</span>
                  <span>技术细节（次要信息）</span>
                ...
                {isLogsLoading ? (
                  <div className="px-5 py-12 text-center text-white/50">正在加载日志...</div>
                ) : logs.length === 0 ? (
                  <div className="px-5 py-12 text-center text-white/50">暂无日志</div>
                ) : (
                  <div className="divide-y divide-white/8">
                    {logs.map((log) => (
                      <div key={log.id} className="grid grid-cols-[1.15fr_1fr_3.3fr_1.8fr_0.7fr_1.1fr] items-center gap-4 px-5 py-4 text-sm">
                        <div className="text-white/55">{formatDate(log.createdAt)}</div>
                        <div className="truncate text-white/75" title={log.usernameSnapshot || undefined}>
                          {log.usernameSnapshot || (log.userId === null ? '系统' : '已删除用户')}
                        </div>
                        <div className="min-w-0 break-words font-semibold leading-6 text-cyan-50" title={log.description || undefined}>
                          {log.description || '历史日志暂无中文说明'}
                        </div>
                        <div className="min-w-0 space-y-1 text-xs leading-5 text-white/45">
                          <div className="break-all"><span className="text-white/30">动作代码：</span>{log.action}</div>
                          <div className="break-all"><span className="text-white/30">HTTP：</span>{log.method} {log.path}</div>
                        </div>
                        <div>
                        ...
                        <div className="break-all text-white/55">{log.ipAddress || '-'}</div>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="flex items-center justify-between gap-3 border-t border-white/10 px-5 py-4 text-sm text-white/55">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 font-bold text-white/70 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-35"
                >
                  上一页
                </button>
                <button
