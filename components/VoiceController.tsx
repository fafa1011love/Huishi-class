...
const COMMAND_PATTERNS: [RegExp, string, (ctrl: ControlRefs) => void, boolean?][] = [
  ...
  [/转(九十度|90度|一下|几圈|一圈)|右?转(九十度|90度)|左?转(九十度|90度)|\bturn(?:\s+(?:90|right|left|around|once))\b|\bspin\s+(?:once|90)\b/i, 'turn_burst', (c) => { console.log('[VoiceCmd] turn_burst fired'); c.rotationLocked = false; c.voiceRotationActive = false; c.rotationVelocity = { x: 0, y: 3.6 }; setTimeout(() => { if (!c.voiceRotationActive) c.rotationVelocity = { x: 0, y: 0 }; }, 400); }],
  [/解锁旋转|解除锁定|取消锁定|恢复旋转|继续旋转|继续转|开始转|转圈|旋转|一直转|转起来|转动|\b(rotate|rotation|keep[ -]?turning|continuously|start[ -]?spin|keep[ -]?spin)\b/i, 'spin', (c) => { console.log('[VoiceCmd] spin fired'); c.rotationLocked = false; c.voiceRotationActive = true; c.rotationVelocity = { x: 0, y: 2.1 }; }],
];
