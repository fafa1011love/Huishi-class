...
const XiaozhiAssistant: React.FC<XiaozhiAssistantProps> = ({
  ...
  return (
    <div className="w-full rounded-2xl border border-cyan-300/20 bg-[#061626]/82 p-3 text-white shadow-2xl shadow-cyan-950/40 backdrop-blur-xl">
      <div className="flex gap-3">
        <div className="relative flex h-[112px] w-[96px] shrink-0 items-center justify-center overflow-hidden rounded-xl border border-cyan-300/18 bg-[radial-gradient(circle_at_50%_40%,rgba(47,177,255,0.12),rgba(2,8,18,0.78)_72%)]">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm font-black text-cyan-50">小智</span>
            <span className="ml-auto h-2 w-2 rounded-full" style={{ backgroundColor: meta.accent }} />
          </div>
          <div className="mt-1 text-[10px] font-bold text-cyan-200/70">
            {voiceActive ? '语音在线 · ' : ''}{meta.label}
          </div>
          <p className="mt-2 line-clamp-4 min-h-[48px] text-[11px] font-medium leading-relaxed text-slate-300">
      ...
      <button
        ...
        className={`mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-xl border text-xs font-bold transition active:scale-[0.98] ${voiceInputDisabled
          ...
          : 'border-cyan-300/25 bg-cyan-400/12 text-cyan-200 hover:border-cyan-300/45 hover:bg-cyan-400/20'
        }`}
